...
save_path = strcat('run_logs/', string(current_time, 'yyyyMMdd_HHmmss'), '_drag_ki');
mkdir(save_path);
...
u0 = [0.3; 0.1; 0.05];    % stiffness K hold constant in this simulation
...
y_init(1, :) = x0;
K_traj_initial = u0;
% Kx = open('run_logs/20250816_134753_mkt_0.4_2.0/x_it_99_0.424974.mat').x;
% K_traj_initial = reshape(Kx(p.n_colloc*p.n_x+1:end), p.n_links-1, p.N_total);
...
for i = 2:p.n_steps
    ...
    forces_y_all(i, 1) = calculate_physics_thrust(xk, p);
...
subplot(1,2,1);
plot_thrust(forces_y_all, p);


subplot(1,2,2);
plot_angle_evolution(theta_all, p);
...
% stiffness bounds
for i=1:p.N_total
    idxk = p.n_x*p.n_colloc+(p.n_links-1)*(i-1);
    for j=1:p.n_links-1
        lb(idxk+j) = p.K_min(j);
        ub(idxk+j) = p.K_max(j);
    end
...
% n_sym_constrs = (p.N_u/2*p.n_cycles-1)*(p.n_links-1);
Aeq = zeros(p.n_x, length(decision_vars));
beq = zeros(p.n_x, 1);
...
%         for j=1:p.n_links-1
%             cidxj=3*(cidx-1)+j+p.n_x;
%             kid1j=p.n_x*p.n_colloc+(p.n_links-1)*(kid1-1)+j;
%             kid2j=p.n_x*p.n_colloc+(p.n_links-1)*(kid2-1)+j;
%             Aeq(cidxj, kid1j) = 1.0;
%             Aeq(cidxj, kid2j) = -1.0;
%             beq(cidxj) = 0.0;
%         end
...
% end
% Ki constraint
% n_ki_constrs = (p.N_total-1)*(p.n_links-1);
% AeqKi = zeros(n_ki_constrs, length(decision_vars));
% beqKi = zeros(n_ki_constrs, 1);
% for i = 1:p.n_links-1
%     for j = 1:p.N_total-1
%         idx = p.n_x*p.n_colloc + i + (j-1)*3;
%         nidx = idx+3;
%         rid = (i-1)*(p.N_total-1)+j;
%         AeqKi(rid, idx) = 1;
%         AeqKi(rid, nidx) = -1;
%         beqKi(rid) = 0.0;
%     end
...
% Aineq = zeros(6*p.N_total, length(decision_vars));
% bineq = zeros(6*p.N_total, 1);
Aineq = zeros(1, length(decision_vars));
bineq = zeros(1, 1);
% for i = 1:3
%     for j = 1:p.N_total
%         idx = p.n_x*p.n_colloc + i + (j-1)*3;
...
function [c, ceq] = dynamics_constraints(x, p)
    ...
    K_traj = reshape(x(p.n_colloc*p.n_x+1:end), p.n_links-1, p.N_total);
...
function obj = thrust_objective(x, p)
    ...
    parfor i=1:p.n_colloc
        x_k = states(i, :)';
        forces_y_all(i, 1) = calculate_physics_thrust(x_k, p);
    end
...
function p = defaultParams()
    ...
    p.f = 1.0;       % Frequency (Hz)
    p.A = 0.8;       % Amplitude
    p.phi0 = 0;      % Phase
    p.bias = deg2rad(0);      % Bias
    p.duty_ratio = 0.4;  % Power stroke duty ratio (0.5 means equal power and recovery stroke times)
    ...
    p.cd = 0.49;              % Drag coefficient (constant)
    p.cf = 0.1;              % Friction coefficient
    p.cm = 1.0;              % Added mass coefficient
    ...
    p.N_u = 16;  % Number of control points per cycle
...
function appendage_animation(theta_all, uk, p)
    ...
    xlabel('x (m)', 'Color', 'w', 'FontSize', 12);
    ylabel('y (m)', 'Color', 'w', 'FontSize', 12);
    title(sprintf('Flexible Appendage Animation (Duty Ratio = %.2f)', p.duty_ratio), 'Color', 'w', 'FontSize', 14);
...
function plot_thrust(thrust_y, p)
    % 绘制总推力
...
function residual = body_constraint_residual(q, t, p)
    ...
    [theta1_ref, ~, ~] = asymmetric_driving_signal(t, p);
...
function forces = hydrodynamic_forces(v, p)
    % Enhanced hydrodynamic force model including drag and added mass effects
    forces = zeros(length(v), 1);
    ...
    for i = 1:p.n_links
        ...
        if norm_v > 1e-10  % Avoid division by zero
            ...
            theta_i = atan2(v_y, v_x);
            
            % Drag force components
            F_drag_x = -0.5 * p.cd * p.rho * l * w * norm_v * v_x;
            F_drag_y = -0.5 * p.cd * p.rho * l * w * norm_v * v_y;
            
            % Rotational drag torque
            T_drag = -0.5 * p.cf * p.rho * (l^3 * w / 12) * abs(omega) * omega;
            
            forces(idx, 1) = F_drag_x;
            forces(idx+1, 1) = F_drag_y;
            forces(idx+2, 1) = T_drag;
        end
    end
end
...
function residual = stationarity_residual(x_kp1, x_k, u_k, t, dt, p)
    ...
    stationarity_residual_kp1 = M*v_kp1 + (dt/2)*potential_grad(q_mid, K_k) - ...
        dt*dc_dq_k'*lambda_k - (dt/2)*hydrodynamic_forces(v_kp1, p);
    
    stationarity_residual_k = M*v_k - (dt/2)*potential_grad(q_k - 0.5*dt*v_k, K_k) + ...
        (dt/2)*hydrodynamic_forces(v_k, p);
...
function total_thrust_y = calculate_physics_thrust(x, p)
    v = x(p.n_q+1:p.n_q+p.n_v);
    forces = hydrodynamic_forces(v, p);
    y_forces = zeros(p.n_links, 1);
    for i = 1:p.n_links
        y_forces(i, 1) = forces(3*i-1, 1);
    end
    total_thrust_y = sum(y_forces);
end

%% ============ Helper Functions ============
function [theta, is_power_stroke, dtheta] = asymmetric_driving_signal(t, p)
    f = p.f;
    A = p.A;
    phi0 = p.phi0;
    bias = p.bias;
    duty_ratio = p.duty_ratio;
    ...
    t1 = (1-duty_ratio)/2;      % End point of first half recovery
    t2 = t1 + duty_ratio/2;     % End point of first half power
    t3 = t2 + duty_ratio/2;     % End point of second half power
    t4 = 1.0;                   % End point of second half recovery (end of cycle)
    
    % Calculate phase and stroke type based on time position
    if cycle_pos < t1
        % First half of recovery stroke (rising)
        norm_pos = cycle_pos / t1;
        phase = norm_pos * (pi/2);
        is_power_stroke = 0;
    elseif cycle_pos < t2
        % First half of power stroke (falling)
        norm_pos = (cycle_pos - t1) / (t2 - t1);
        phase = (pi/2) + norm_pos * (pi/2);
        is_power_stroke = 1;
    elseif cycle_pos < t3
        % Second half of power stroke (falling)
        norm_pos = (cycle_pos - t2) / (t3 - t2);
        phase = pi + norm_pos * (pi/2);
        is_power_stroke = 1;
    else
        % Second half of recovery stroke (rising)
        norm_pos = (cycle_pos - t3) / (t4 - t3);
        phase = (3*pi/2) + norm_pos * (pi/2);
        is_power_stroke = 0;
    end
    
    % Calculate position and velocity
    theta = A * sin(phase + phi0) + bias;
    
    % Calculate velocity (note the time compression/expansion effect)
    if cycle_pos < t1
        dtheta = A * cos(phase + phi0) * ((pi/2) / (t1 * T));
    elseif cycle_pos < t2
        dtheta = A * cos(phase + phi0) * ((pi/2) / ((t2 - t1) * T));
    elseif cycle_pos < t3
        dtheta = A * cos(phase + phi0) * ((pi/2) / ((t3 - t2) * T));
    else
        dtheta = A * cos(phase + phi0) * ((pi/2) / ((t4 - t3) * T));
    end
    
    % Confirm stroke type based on velocity direction
    is_power_stroke = (dtheta < 0);
end
...
function K_init = initialStiffness(K_nominal, high_low_ratio, params)
    % 修正：避免参数名冲突
    K_min = params.K_min;
    K_max = params.K_max;
    
    % Initial stiffness values (to be optimized)
    K_init = zeros(length(K_nominal), params.N_total);
    for i = 1:length(K_nominal)
        half_cycle_time = params.cycle_time / 2.0;
        half_power_time = half_cycle_time * params.duty_ratio;
        t1 = half_cycle_time - half_power_time;
        t2 = half_cycle_time + half_power_time;
        poly_coeff = polyfit([t1, half_cycle_time, t2], [0.0, 1.0, 0.0], 2);
        t = mod(params.t_control, params.cycle_time);
        K_init(i,:) = K_nominal(i) + high_low_ratio*K_nominal(i)*max(polyval(poly_coeff, t), 0);
        
        % Ensure within bounds
        K_init(i, K_init(i,:) < K_min(i)) = K_min(i);
        K_init(i, K_init(i,:) > K_max(i)) = K_max(i);
    end
end
...
function K_effective = interp_stiffness(t, t_control, K_traj)
    ...
    % K_effective = zeros(size(K_traj, 1), 1);
    % for i = 1:size(K_traj, 1)
    %     % Use piecewise cubic Hermite interpolation for smoothness
    %     K_effective(i) = interp1(t_control, K_traj(i,:), t_mod, 'pchip', 'extrap');
    % end
    K_effective = K_traj;
end
