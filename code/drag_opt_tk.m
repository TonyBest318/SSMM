...
save_path = strcat('run_logs/', string(current_time, 'yyyyMMdd_HHmmss'), '_drag_skt');
mkdir(save_path);
...
u0 = [0.03; 0.03; 0.03];    % stiffness K hold constant in this simulation
...
y_init(1, :) = x0;
K_traj_initial = initialStiffness(u0, 10.0, p);
...
function K_effective = interp_stiffness(t, t_control, K_traj)
    ...
    for i = 1:3
        ...
        K_effective(i) = interp1(t_control, K_traj(1,:), t_mod, 'pchip', 'extrap');
    end
end
