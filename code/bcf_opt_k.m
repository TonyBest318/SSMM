...
save_path = strcat('run_logs/', string(current_time, 'yyyyMMdd_HHmmss'), '_bcf_ki');
mkdir(save_path);
...
y_init(1, :) = x0;
K_traj_initial = u0;
% Kx = open('run_logs/20250820_161258_bcf_1.0/x_it_132_0.342271.mat').x;
...
function rgb = get_rdbu_r_color(x, xmin, xmax)
    % Get RGB color from RdBu_r colormap for value x in range [xmin, xmax]
    %
    % Parameters:
    %   x: value to map to color
    %   xmin, xmax: range of your data  
    %   rdbu_r_colormap: Nx3 matrix of RGB values for RdBu_r colormap
    %
    % Returns:
    %   rgb: [R G B] values in range [0, 1]
    
    % Normalize x to [0, 1] range
    if xmax == xmin
        normalized_x = 0.5;  % Middle color if no range
    else
    ...
    rdbu_colormap = [
        0.60392156862745099,  0.0                ,  0.12156862745098039;
    ...
    N = size(rdbu_colormap, 1);
    index = max(1, min(N, round(normalized_x * (N-1) + 1)));
    index = N+1-index;
    
    % Get RGB color
    rgb = rdbu_colormap(index, :);
end
...
function color = get_stiffness_color(stiffness_value, K_min, K_max)
    ...
    color = get_rdbu_r_color(stiffness_value, K_min, K_max);
end
...
function appendage_animation(theta_all, uk, p)
    % 创建动画窗口 - 黑色背景
    anim_fig = figure('Position', [100, 100, 400, 400], 'Name', 'Appendage Animation', 'Color', 'k');
    ...
    for i = 1:3
        ...
        joint_sizes(i) = get_stiffness_marker_size(0.1, p.K_min(i), p.K_max(i), 6, 8);
    end
